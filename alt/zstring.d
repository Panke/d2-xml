...
Advantages ?
Fun of debugging crashes from obscure memory errors. It does copy on write, if reference count is > 1.
This is absolutely thread unsafe,
Array!T hould never be shared between threads. I will never be capable of writing a thread-safe version.
For interthread communication and sharing use immutable data.
...
private {
	...
	/* return new clean zero'd memory */
...
struct Array(T)
{
	...
	this(this)
	{
		if (ptr_)
		{
			if (length_ == 0)
			{
				// cannot share zero length array.
				ptr_ = null;
				return;
			}
	...
	private void deallocate(bool del=false)
	{
		if (ptr_)
		{
			...
			else if (pap.refcount_ == 0)
			{
				debug
					writeln("bug refcount == 0 ");
			}
			ptr_ = null;
			length_ = 0;
		}
	...
	/* get a unique reference copy of current buffer, if more than one reference */
	private final void copyOnWrite()
	{
		...
		{
			reserve(length_,true); // this will do all the work
		}
	}
	...
	/*  Assign the entire buffer
		There is a possibility that the buffer is part of the current data.
		Check, if reference count is 1.  Assigning part of self to self.
	...
	*/
	void assign(const(T)* buf, size_t slen)
	{
		...
		if (ptr_)
		{
			...
			// safe to call deallocate
			deallocate(false);
		}
		// ptr_ should be guarenteed invalid
		ArrayPrefix* npap =  createCapacity!T(slen);
		ptr_ = cast(T*) (npap+1);
    ...
    final void clear()
    {
		...
		*/
		...
		if (pap.refcount_ > 1)
		{
			deallocate(true); // others have control
		}
		else
			// opportunity to reuse capacity
		{
			if (length_ > 0)
				destroy_data!T(ptr_,length_);
			length_ = 0;
    ...
    */
	...
	int opCmp(ref const SameTypeArray ro) const
	{
...
struct KeyValRec(K,V)
{
	...
	bool opEquals(ref const SameType ro) const
	{
	 ...
	 int opCmp(ref const SameType s) const
	{
