 ...
 * Modified by Michael Rynn for experiments and statistics. In particular, to
 * store the capacity at beginning of block, so that calls to free can easily detect the amount of
 * memory being freed.
 */
module gc.gc;

import std.stdint, core.stdc.stdio;
class GCStats {
	private __gshared ulong allocCt = 0;
	private __gshared ulong freeCt = 0;
	private __gshared ulong allocCall = 0;
	private __gshared ulong freeCall = 0;

	synchronized static void inc(size_t m)
	{
		allocCt += m;
		allocCall += 1;
	}
	synchronized static void dec(size_t m)
	{
		freeCt += m;
		freeCall += 1;
	}
	synchronized static void getStats(ref ulong allocated, ref ulong freed, ref ulong callAlloc, ref ulong callFree)
	{
		allocated = allocCt;
		freed = freeCt;
		callAlloc = allocCall;
	...
	{
		ulong used, freed, callA, callF;

		GCStats.getStats(used,freed, callA, callF);
		printf("Allocated = %u, Freed = %u, Diff = %u,  call = %u, free = %u\n", cast(uint)used, cast(uint)freed, cast(uint)(used - freed), cast(uint)callA, cast(uint)callF);
	}
}
private
{
...
extern (C) void* gc_malloc( size_t sz, uint ba = 0 )
{
    if( proxy is null )
    {
		auto s2 = sz + size_t.sizeof;
        void* p = malloc(s2);
        if(p is null )
            onOutOfMemoryError();
		auto q = cast(size_t*)p;
		*q = sz;
		p = q+1;
		GCStats.inc(sz);
        return p;
...
extern (C) void* gc_realloc( void* p, size_t sz, uint ba = 0 )
{
    if( proxy is null )
    {
		...
		auto s2 = (sz > 0) ? sz + size_t.sizeof : 0;
        p = realloc( p, sz );
