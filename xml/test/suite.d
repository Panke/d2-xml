...
import alt.buffer;
import xml.dom.domt, xml.util.read, xml.ixml;
import xml.xmlerror, xml.parse.input, xml.parse.domt;

import std.stdint, std.path, std.stdio, std.conv, std.variant, std.string;
template XMLTESTS(T)
{
