...
module std.xmlp.xmlparse;

import alt.buffer,  std.xmlp.charinput, std.xmlp.error, std.xmlp.subparse;
import std.xmlp.feeder, std.xmlp.parseitem, std.xmlp.xmlchar;


import std.conv, std.string,  std.utf, std.variant;
import std.file, std.path;

import std.xmlp.dtdtype, std.xmlp.dtdvalidate, std.xmlp.entitydata, std.xmlp.nodetype;
...
version(TagNesting)
{
	import std.xmlp.tagnest;
	version=StrAllocation;
}

version(TrackCount)
{
	string trackspot(int line)
	{
		return format("%s %s",__FILE__, line);
	}
}
...
{
package:
    Buffer!char          tagScratch_;
    Buffer!char	         attScratch_; // attribute normalisation
    Buffer!char          entityName_;
    Buffer!char          content_;
    version(speedBump)
		Buffer!char          normalizeBuf_;
...
public:	
    ...
    private void init2()
    {
		...
		version(TrackCount)
		{
			attScratch_.setid(trackspot(__LINE__));
			content_.setid(trackspot(__LINE__));
			scratch_.setid(trackspot(__LINE__));
			tagScratch_.setid(trackspot(__LINE__));
		}
...
protected:
    ...
    Buffer!char	scratch_; // stay around buffer for non re-entrent functions
    ...
    Buffer!SavedContext	contextStack_;
    ...
    version(TagStack)
    {
        Buffer!string      tagStack_; // check and resuse tag strings
        string            lastCloseTag_;         // for starting tag thats the same as previous start tag
    }

	version(TagNesting)
	{
		Buffer!(TagNest!char)	tagNest_;
		TagNest!char			rootTag_;
	}
    ...
    /*
    protected void copyAttributes(XmlReturn r)
    {
    ...
    */
    ...
    void doPushContext(string s, bool scoped, EntityData ed)
    {
		...
		auto slen = contextStack_.length + 1;
		contextStack_.length = slen;
        saveContext(*contextStack_.last);
        if (slen == 1)
            stackElementDepth_ = elementDepth_;
        else
            stackElementDepth_ += elementDepth_;
...
public:
    ...
    /** Very testy function.  Attributes are not acted on until the whole declaration is parsed!
    	...
    	*/
    bool doXmlDeclaration(XmlReturn ret)
    {
    ...
    final bool getAttributeValue(ref Buffer!char app)
    {
        munchSpace();
        dchar test;
        if (empty || (front != '='))
    ...
    final bool unquoteValue(ref Buffer!char app)
    {
    ...
    protected final void expectEntityName(ref Buffer!char ename)
    {
        if (!getXmlName(ename))
    ...
    protected bool getXmlName(ref Buffer!char sbuf)
    {
        if (empty)
            return false;
    ...
    void handleProcessInstruction(XmlReturn iret)
    {
    ...
    protected bool doProcessingInstruction(XmlReturn ret, uint spaceCt = 0)
    {
        if (!getXmlName(scratch_))
    ...
    protected final void getPIData(ref Buffer!char app)
    {
        dchar  test  = 0;
    ...
    protected final bool getXmlNmToken(ref Buffer!char cbuf)
    {
        if (empty)
            return false;
        if ( !(isNameCharFn_(front) || isNameCharFifthEdition(front)) )
    ...
    final void parseComment(ref Buffer!char app)
    {
        dchar  test  = 0;
        app.length = 0;
        frontFilterOn();
    ...
    bool doCommentContent(XmlReturn ret)
    {
        if ((state_ != PState.P_PROLOG) && (state_ != PState.P_EPILOG))
            state_ = PState.P_DATA;
    ...
    protected final bool doStartTag(XmlReturn r)
    {
        string attName;
        string attValue;
		...
		version(TagNesting)
		{
			auto parentTag = (tagNest_.length > 0) ? tagNest_.back() : null;
			TagNest!char childTag;

			if (parentTag is null)
			{
				r.scratch = strAlloc_.alloc(tagScratch_.peek);
				childTag = new TagNest!char(r.scratch);
				rootTag_ = childTag;
			}
			else {
				childTag = parentTag.findChild(tagScratch_.peek);
				if (childTag is null)
				{
					r.scratch = strAlloc_.alloc(tagScratch_.peek);
					childTag = new TagNest!char(r.scratch);
        ...
        while (true)
        {
            ...
            case '>': // no attributes, but possible inner content
                ...
                itemCount++;
				// Special dispensation for Jabber Client at root element, because of streaming network issue.
				if (isFragment && elementDepth_==1)
				{
					...
					version(TagNesting)
					{
						tagNest_.put(childTag);
					}
					pendingPop = true;
					return true; 
				}
				// Not in Jabber, can throw a fit if no pending input on popFront
				popFront(); 
				if (empty)
				{
            ...
            case '/':
				...
				version(TagNesting){} else allocTag();
				checkAttributes();
            ...
            default:
				...
				version(TagNesting)
				{
					attName = childTag.findAttributeName(scratch_.peek);
					if (attName is null)
					{
						attName = strAlloc_.alloc(scratch_.peek);
						childTag.addAttributeName(attName);
					}
				}
				else {
                version(StrAllocation)
                ...
                version(StrAllocation)
					attValue = strAlloc_.alloc(scratch_.peek);
				else
    ...
    public override intptr_t tagDepth()
    {
    ...
    protected final bool doEndTag(XmlReturn ret)
    {
        state_ = PState.P_DATA;
        dchar  test;
        ...
        if (getXmlName(tagScratch_))
        {
			...
			version(TagNesting)
			{
				if (tagNest_.length > 0)
				{
					auto childTag = tagNest_.movePopBack();
					if (childTag.tag_ != tagScratch_.peek)
						throwErrorCode(ParseErrorCode.ELEMENT_NESTING);
					ret.scratch = childTag.tag_;
				}
			}
            else
            {
    ...
    protected final bool doContent(XmlReturn ret)
    {
        content_.length = 0;
    ...
    final bool doCDATAContent(XmlReturn ret)
    {
        state_ = PState.P_DATA;
        scratch_.length = 0;
        frontFilterOn();
    ...
    */
    bool textReplaceEntities(bool isInternal, string src, ref string opt, ref StringSet stk)
    {
        ...
        bool insertPEContent(const(char)[] peName)
        {
            ...
            /*if (isInternal)
            return push_error("parameter entity in internal subset markup");
            */
