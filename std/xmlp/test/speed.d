...
void timeSerious()
{
	...
	TagHandlerSet	graphHandlers;
	TagHandlerSet	plotHandlers;
	TagHandlerSet	rootHandlers;

	{   
		Axis		  ax;
		axisHandlers["label",XmlResult.STR_TEXT] = (XmlReturn ret)
		{
			ax.label_ = ret.scratch;
		};
		axisHandlers["unit",XmlResult.STR_TEXT] = (XmlReturn ret)
		{
			ax.unit_ = ret.scratch;
		};
		rootHandlers["axis", XmlResult.TAG_START] = (XmlReturn ret)
		{
			ax = new Axis(); 	
			ax.id_ = ret.attr["id"];
			axes[ax.id_] = ax;

			visitor.pushHandlerSet(axisHandlers);
			visitor.parseDocument(-1);
	...
	
	{
		...
		plotHandlers["d2",XmlResult.TAG_SINGLE] = pointCollect;
		plotHandlers["d2",XmlResult.TAG_START] = pointCollect;

		graphHandlers["plot",XmlResult.TAG_START] = (XmlReturn ret) {
			auto aplot = graph.new Plot();
			graph.plots_ ~= aplot;
			plot = aplot;

			aplot.title_ = ret.attr["id"];
			aplot.xdata_ = new Series();
			aplot.ydata_ = new Series();

			visitor.pushHandlerSet(plotHandlers);
			visitor.parseDocument(-1);
			visitor.popHandlerSet();
		};
		//Assume axis precedes
		rootHandlers["graph", XmlResult.TAG_START] = (XmlReturn ret)
		{
			auto g = new Graph();
			graphs ~= g;
			graph = g;

			g.xaxis_ = axes[ret.attr["x"]];
			g.yaxis_ = axes[ret.attr["y"]];

			visitor.pushHandlerSet(graphHandlers);
			visitor.parseDocument(-1);
			visitor.popHandlerSet();	
		};
	}
		
	visitor.handlerSet = rootHandlers;
...
void testNewStuff()
{
    ...
    // Used to gather all text inside C element
    ...
    tv.defaults[XmlResult.STR_TEXT] = textDg;
    tv.defaults[XmlResult.STR_CDATA] = textDg;
    tv.defaults[XmlResult.STR_PI] = piDg;
    tv.defaults[XmlResult.STR_COMMENT] = textDg;

    tv.defaults[XmlResult.XML_DEC] = (XmlReturn ret)
    {
		foreach(n,v;ret.attr)
			writefln("%s='%s'",n, v);
    };

	Buffer!DefaultTagBlock	saveDefaults; // nested stack for saving default handler set
	auto bce = new ardb.ArrayDomBuilder();
	TagHandlerSet	tset;

	tset["C",XmlResult.TAG_SINGLE] = (XmlReturn ret)
	{
    ...
    tset["C",XmlResult.TAG_START] = (XmlReturn ret)
    {
	...
	tset["C",XmlResult.TAG_END] = (XmlReturn ret)
	{
		if (saveDefaults.length > 0)
			tv.defaults = saveDefaults.movePopBack();
		auto elem = bce.root;
	...
	tset["B",XmlResult.TAG_START] = (XmlReturn ret)
	{
		writeln("B Start");
	};

	tv.defaults[XmlResult.TAG_START] = (XmlReturn ret)
    {
        writeln("Unhandled tag start ",ret.name);
    };

	tv.defaults[XmlResult.TAG_END] = (XmlReturn ret)
    {
        writeln("Unhandled tag end ",ret.name);
    };
	tv.handlerSet = tset;
    tv.parseDocument(0);
    writefln("%s",allText);
...
void books2collect(string s)
{
    ...
    TagHandlerSet bookcb;

	catalog["book",XmlResult.TAG_START] = (XmlReturn ret) {
		book.id = ret.attr["id"];
		visitor.pushHandlerSet(bookcb);
	};
	...
	bookcb["author", XmlResult.STR_TEXT] = (XmlReturn ret) {
		book.author = ret.scratch;
	};
	bookcb["title", XmlResult.STR_TEXT] = (XmlReturn ret) {
		book.title = ret.scratch;
	};
	bookcb["genre", XmlResult.STR_TEXT] = (XmlReturn ret) {
		book.genre = ret.scratch;
	};
	bookcb["price",XmlResult.STR_TEXT] = (XmlReturn ret)
	{
		book.price = ret.scratch;
	};
	bookcb["publish_date",XmlResult.STR_TEXT] = (XmlReturn ret)
	{
		book.pubDate = ret.scratch;
	};
	bookcb["description",XmlResult.STR_TEXT] = (XmlReturn ret)
	{
...
void emptyDocElement()
{
    ...
    tset["main", XmlResult.TAG_START] = (XmlReturn ret)
    {
        // main item
...
void testTicket8()
{
    ...
    bool testTicket(string src)
    {
        try
        {
			...
			bset["B", XmlResult.TAG_START] = (XmlReturn ret)
			{
				btext = null;
			};

			bset["B", XmlResult.STR_TEXT] = (XmlReturn ret)
			{
				btext ~= ret.scratch;
			};

            bset["B",XmlResult.TAG_END] = (XmlReturn ret)
            {
				assert (btext == "\nhello\n\n", "Collect text only");
            };
			tv.handlerSet = bset;
