...
import alt.buffer;
import std.xmlp.entity;
import std.xmlp.feeder;
import std.stream;
import std.utf;
import std.string;
import std.array;
...
extern (C) bool getSystemEntity(IXMLParser vp,string sysid, ref string opt, ref string baseDir);
...
[Definition: For an external entity, the replacement text is the content of the entity,
    ...
    void parseEntity()
    {
        ...
        else
        {
            ...
            if (estr.length > 0)
            {
                if (startsWith!("a==b")(estr.peek,"<?xml"))
                {
    ...
    bool getSystemLiteral(ref Buffer!char opt)
    {
        if (empty)
            doUnexpectedEnd();
...
protected:
    ...
    override bool doDocType(XmlReturn iret)
    {
        inDTD_ = true;
