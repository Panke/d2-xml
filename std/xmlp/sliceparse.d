...
class XmlStringParser : StringSource, IXMLParser
{
    ...
    version(speedBump)
        Buffer!char	normValue_;
    Buffer!dchar	endCheck_;
    ...
    version(TagStack)  Buffer!string  tagStack_;
    ...
    {
        this();
		initSource(sourceText);
    }

	void initSource(string sourceText)
	{
        source_ = sourceText;
    ...
    public bool doDocType(XmlReturn ret)
    {
    ...
    protected intptr_t elementDepth()
    {
    ...
    final bool doCommentContent(XmlReturn ret)
    {
...
/// Very testy function.  Attributes are not acted on until the whole declaration is parsed!
    ...
    bool doProcessingInstruction(XmlReturn ret, uint spaceCt = 0)
    {
    ...
    final bool doStartTag(XmlReturn r)
    {
        state_ = PState.P_DATA;
<<<<<<< TREE
...
>>>>>>> MERGE-SOURCE
    ...
    final bool doEndTag(XmlReturn ret)
    {
        state_ = PState.P_DATA;
        dchar  test;
		setSingleCharPos(srcIX_+2);
	...
	final bool doBang(XmlReturn ret)
	{
		markupDepth_++;
		setSingleCharPos(srcIX_ + 2);
    ...
    final bool doContent(XmlReturn ret)
    {
        ...
        bool returnTextContent(int negadjust)
        {
			...
			auto endText = srcIX_-negadjust;
            if (isSubstitute)
                ret.scratch = content_.idup;
            else 
                ret.scratch = source_[contentIX..endText];
            itemCount++;
            return true;
        }
			...
			if (front == '<')
			{
                ...
                default:
					...
					if(getXmlName(tagScratch))
					{
                        ...
                        if (inCharData)
						{
							cacheSource_ = source_[tagPos_..$];
                            return returnTextContent(srcIX_ - tagPos_);
						}
						else {
							return doStartTag(ret);
						}
