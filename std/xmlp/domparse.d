...
module std.xmlp.domparse;

import std.xmlp.linkdom;
import std.xmlp.elemvalidate;
...
import std.xmlp.entitydata, std.xmlp.validate;
import alt.buffer;
import std.utf;
...
public:
	...
	void popTag(XmlReturn ret)
	{
		...
		level = parseStack_.length > 0 ? parseStack_.movePopBack() : ParseLevel.init;
		parent_ = (level.elem is null) ? doc_ : level.elem;
	}
	void emptyTag(XmlReturn ret)
	{
...
IXMLParser parseXmlString(T)(const(T)[] src)
{
    auto sf = new SliceFill!T(src);
...
IXMLParser parseXmlStringValidate(T)(const(T)[] src)
{
    auto sf = new SliceFill!T(src);
